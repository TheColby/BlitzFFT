...
type FnCudaMemcpyAsync = unsafe extern "C" fn(
    *mut std::ffi::c_void,
...
impl CudaFftBackend {
    pub fn try_init() -> Option<Self> {
        ...
        let fns = CudaFns {
            ...
            cuda_stream_sync: sym!(
                cuda_lib,
                b"cudaStreamSynchronize\0",
                FnCudaStreamSynchronize
            ),
            cuda_device_sync: sym!(
                cuda_lib,
                b"cudaDeviceSynchronize\0",
                FnCudaDeviceSynchronize
            ),
    ...
    unsafe fn cf(&self, r: CufftResult, op: &str) -> Result<()> {
        if r == CUFFT_SUCCESS {
            ...
            Err(anyhow!("cuFFT error {} in {}", r, op))
        }
    }
    ...
    unsafe fn get_plan(&self, n: usize, batch: usize) -> Result<(CufftHandle, CudaStream)> {
        ...
        self.cc(
            (self.fns.cuda_stream_create)(&mut stream),
            "cudaStreamCreate",
        )?;
    ...
    unsafe fn ensure_bufs(&self, batch: usize, n: usize) -> Result<()> {
        ...
        self.cc(
            (self.fns.cuda_malloc)(&mut d_out, out_bytes),
            "malloc d_out",
        )?;
        self.cc(
            (self.fns.cuda_malloc)(&mut d_mag, mag_bytes),
            "malloc d_mag",
        )?;
        self.cc(
            (self.fns.cuda_malloc)(&mut d_win, win_bytes),
            "malloc d_win",
        )?;
    ...
    unsafe fn upload_window(&self, n: usize, stream: CudaStream) -> Result<()> {
        ...
        self.cc(
            (self.fns.cuda_host_alloc)(&mut h_win, bytes, 0),
            "hostalloc h_win",
        )?;
...
impl FftBackend for CudaFftBackend {
    fn name(&self) -> &str {
        "cuFFT (CUDA) — plan-cached + async streams (v2)"
    }
    ...
    fn compute_batch(&self, frames: &[&[f32]], fft_size: usize) -> Result<Vec<FftFrame>> {
        ...
        unsafe {
            ...
            self.cc(
                (self.fns.cuda_memcpy_async)(b.d_in, b.h_in as _, batch * n * 4, H2D, stream),
                "memcpy H2D",
            )?;
            ...
            self.cc(
                (self.fns.cuda_memcpy_async)(b.h_mag as _, b.d_out, out_bytes, D2H, stream),
                "memcpy D2H",
            )?;
